...
export const createProjectValidation = z.object({
  body: z.object({
    ...
    technologies: z
      .array(z.string().min(1, { message: 'Technology name cannot be empty' }))
      .nonempty({ message: 'At least one technology is required' }),
    liveUrl: z.string().url({ message: 'Live URL must be a valid URL' }),
    clientSideUrl: z
      .string()
      .url({ message: 'Client-side URL must be a valid URL' })
      .optional(),
    serverSideUrl: z
      .string()
      .url({ message: 'Server-side URL must be a valid URL' })
      .optional(),
